...
with zipfile.ZipFile(dist / "jev-android-0.3.1-source.zip", "w", zipfile.ZIP_DEFLATED) as archive:
...
with zipfile.ZipFile(dist / "jev-android-0.3.1-maven.zip", "w", zipfile.ZIP_DEFLATED) as archive:
    for path in sorted(repository.rglob("*")):
        if path.is_file():
            archive.write(path, Path("jev-maven") / path.relative_to(repository))
shutil.copy2(root / "sample/build/outputs/apk/debug/sample-debug.apk", dist / "jev-android-demo-0.3.1.apk")
